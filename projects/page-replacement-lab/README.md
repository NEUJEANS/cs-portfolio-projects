...
- includes a `study-wsclock-modes` workflow that sweeps a frame-budget range and summarizes where adaptive WSClock helps, ties, or regresses against the best fixed window
- leaves room for future extensions like adaptive-vs-fixed gallery dashboards, richer narrative trace write-ups, or more detailed async-cleaner simulations
...
- study adaptive-vs-fixed WSClock outcomes across a frame-budget range with `study-wsclock-modes`, exporting Markdown / SVG / CSV / JSON / text artifacts for portfolio slides
...
### Study adaptive vs fixed WSClock across a frame-budget sweep
```bash
python3 projects/page-replacement-lab/page_replacement_lab.py study-wsclock-modes --min-frames 2 --max-frames 6 \
  --benchmark adaptive-phase-turnover \
  --min-window 1 --max-window 9 --segment-length 8 \
  --markdown-out docs/artifacts/page-replacement-lab/wsclock-frame-study/adaptive-phase-turnover-frame-budget-study.md \
  --svg-out docs/artifacts/page-replacement-lab/wsclock-frame-study/adaptive-phase-turnover-frame-budget-study.svg \
  --csv-out docs/artifacts/page-replacement-lab/wsclock-frame-study/adaptive-phase-turnover-frame-budget-study.csv \
  --json > docs/artifacts/page-replacement-lab/wsclock-frame-study/adaptive-phase-turnover-frame-budget-study.json
```

Use `study-wsclock-modes` when you want one frame-range report that says where adaptive WSClock wins, ties, or loses instead of showing only a single frame budget.
...
- `docs/artifacts/page-replacement-lab/wsclock-frame-study/adaptive-phase-turnover-frame-budget-study.{md,svg,csv,json,txt}` — committed frame-budget sweep bundle that shows where adaptive WSClock helps across multiple frame counts
- `docs/artifacts/page-replacement-lab/wsclock-frame-study/db-hotset-scan-frame-budget-study.{md,svg,csv,json,txt}` — committed counterexample frame-budget sweep bundle that shows the honest tie-or-regression side of adaptive WSClock
...
- show how a frame-budget sweep changes that story by surfacing where adaptive WSClock helps, where it only ties, and where a tuned fixed window still wins
...
## Future improvements
- fold adaptive-vs-fixed WSClock frame-budget studies into the aggregate dashboard or gallery so the multi-frame results are visible without opening separate artifact bundles
