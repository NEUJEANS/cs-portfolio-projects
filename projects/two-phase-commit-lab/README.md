...
## Features
- JSON scenario validation for participant plans, votes, second-phase delivery quirks, and coordinator crash/recovery settings
- deterministic 2PC simulation with trace output for `PREPARE`, votes, durable decisions, crash points, missed second-phase deliveries, and participant reconnect recovery
- committed sample scenarios for happy-path commit, participant veto abort, coordinator blocking before the decision log, coordinator replay after a durable commit, and participant-side reconnect after a missed `COMMIT`
- Markdown report export for recruiter-friendly artifacts under `docs/artifacts/two-phase-commit-lab/`
- multi-scenario catalog generation that regenerates per-scenario reports and writes a portfolio-friendly landing page comparing outcomes plus reconnect recoveries in one place
...
- `miss` - the participant prepared successfully but misses the first second-phase message and stays in doubt until a retry/reconnect resolves it
...
- add scenario tags or thematic groupings (happy path, blocking, coordinator recovery, participant reconnect) inside the catalog if the sample set grows larger
- add participant-to-participant termination-protocol hints for cases where the coordinator is still unavailable
