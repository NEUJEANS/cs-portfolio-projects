...
class LogAnalyzerTests(unittest.TestCase):
    ...
    def test_format_facet_ranking_gallery_html_groups_rankings_and_links(self):
        ...
        self.assertIn('Top path', html)
        self.assertIn('/api/report', html)
        self.assertIn('Top user agent', html)
    ...
    def test_write_facet_ranking_detail_bundle_emits_index_manifest_slices_and_deterministic_zip(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn('At a glance', index_html)
            self.assertIn('Top path', index_html)
            self.assertIn('/api/report', index_html)
            ...
            self.assertEqual(manifest['slices'][0]['scorecard_highlights'][0]['label'], 'Top path')
            self.assertEqual(manifest['slices'][0]['scorecard_highlights'][0]['value'], '/api/report')
