...
- benchmark reports can filter structured annotations by severity, cap visible cards, and emit an overflow-summary callout when you want a tighter portfolio narrative
...
- release-comparison command that diffs two saved inspection/catalog snapshots so the docs bundle can show how the plugin suite evolved across commits
...
- lightweight `docs-index` landing-page export that scans the committed artifact bundle and links plugin catalogs, plugin docs, inspection diffs, release comparisons, benchmark reports, and annotation-batch presets from one place
...
Compare two saved inspection/catalog snapshots when you want a release-to-release view of added plugins, removed plugins, changed hook contracts, and dataset-family drift:

```bash
python3 projects/mini-mapreduce-lab/mapreduce.py compare-plugin-releases \
  --before docs/artifacts/mini-mapreduce/2026-04-17-plugin-catalog.json \
  --after docs/artifacts/mini-mapreduce/plugin-catalog.json \
  --before-label 2026-04-17 \
  --after-label current \
  --output docs/artifacts/mini-mapreduce/2026-04-17-vs-current-release-comparison.json \
  --report-output docs/artifacts/mini-mapreduce/2026-04-17-vs-current-release-comparison.md \
  --html-output docs/artifacts/mini-mapreduce/2026-04-17-vs-current-release-comparison.html
```
...
python3 projects/mini-mapreduce-lab/mapreduce.py benchmark \
  ...
  --shard-size 30 \
  --reducers 2 4 \
  --report-output launch-day-sessionization-report.md \
  --html-output launch-day-sessionization-report.html
```
...
python3 projects/mini-mapreduce-lab/mapreduce.py benchmark \
  ...
  --shard-size 30 \
  --reducers 2 4 \
  --report-output iot-burst-streaming-window-report.md \
  --html-output iot-burst-streaming-window-report.html
```
...
python3 projects/mini-mapreduce-lab/mapreduce.py benchmark \
  ...
  --shard-size 30 \
  --reducers 2 4 \
  --report-output sensor-backfill-watermark-late-summary-report.md \
  --html-output sensor-backfill-watermark-late-summary-report.html
```
...
python3 projects/mini-mapreduce-lab/mapreduce.py benchmark \
  ...
  --shard-size 30 \
  --reducers 2 4 \
  --annotation-severity risk watch \
  --annotation-limit 1 \
  --annotation-overflow summary \
  --report-output project-week-annotation-view.md \
...
- release-comparison bundle: [`../../docs/artifacts/mini-mapreduce/2026-04-17-vs-current-release-comparison.md`](../../docs/artifacts/mini-mapreduce/2026-04-17-vs-current-release-comparison.md) / [`../../docs/artifacts/mini-mapreduce/2026-04-17-vs-current-release-comparison.html`](../../docs/artifacts/mini-mapreduce/2026-04-17-vs-current-release-comparison.html)
...
The `plugins_sessionization.py` example pushes the runner toward product analytics: the mapper emits `user,timestamp,page` events, the combiner keeps shard-local event batches portable, and the reducer turns each user into a session summary with `session_count`, `total_events`, average session length, and longest-session metrics. Its benchmark families (`default`, `exam-revision`, `launch-day`) make the same runner look useful for clickstream/session-window discussions, and the structured annotations give you interview-ready narratives about cram-week traffic or launch-day war-room behavior.
