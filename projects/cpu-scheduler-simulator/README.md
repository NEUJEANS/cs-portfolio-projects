...
- compares named MLFQ tuning ladders so response-first, balanced, and throughput-first queue policies can be benchmarked side by side
...
- expand compare and benchmark runs with `--mlfq-variant-pack portfolio` to contrast interactive, balanced, and throughput-first MLFQ ladders in one artifact set
...
python3 scheduler.py benchmark --benchmark-family portfolio-batch --algorithms srtf rr mlfq --quantum 2 --context-switch-cost 1 --mlfq-variant-pack portfolio
```
...
python3 scheduler.py benchmark \
  ...
  --output-dir ../../docs/artifacts/cpu-scheduler-simulator/mlfq-tuning-pack
```

The committed benchmark bundles now live under `docs/artifacts/cpu-scheduler-simulator/portfolio-batch/` and `docs/artifacts/cpu-scheduler-simulator/mlfq-tuning-pack/`. Together they provide the broad algorithm-family story plus a focused MLFQ policy-tuning case study with pack-level summaries, `benchmark-heatmap.svg`, per-scenario compare Markdown/HTML/SVG/JSON artifacts, and reproducible workload JSON snapshots.
...
## Next extensions
- named workload-family authoring so custom scenario packs can be saved without editing source
