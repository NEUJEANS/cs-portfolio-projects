...
- [ ] Add a checked-in trace SVG/PNG example for README screenshots
- [ ] Add a short complexity discussion section with best/average/worst-case intuition

## Validation
- [x] Unit tests for delete scenarios
- [x] Regression tests for benchmark-series helpers and CLI artifact writes
- [ ] Add a smoke test for `trace --output` artifact creation
- [ ] Add a lightweight README command audit script for docs drift

## Next slice ideas
- [ ] Plot CSV benchmark output into a small chart artifact for README embedding
- [ ] Add an `explain` mode that narrates why branches were pruned for a single query
