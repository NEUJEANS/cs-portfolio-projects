...
class BranchPredictorLabTests(unittest.TestCase):
    ...
    def test_render_budget_sweep_outputs_include_budget_story(self) -> None:
        ...
        self.assertIn("## Per-workload notes", markdown)
        self.assertIn("SVG matrix callouts", markdown)
        self.assertIn("Matrix callout", markdown)
        self.assertIn("`32→128b`", markdown)
    ...
    def test_cli_budget_sweep_json_writes_markdown_svg_csv_and_trace_dir(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("Winner crossover points", markdown_text)
            self.assertIn("Matrix callout", markdown_text)
            self.assertIn("Winner crossover points", svg_text)
            self.assertIn("Blue flip chips mark the budget cell", svg_text)
